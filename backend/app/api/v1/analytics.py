...
def _ownership_filters(current_user, filter_user_id: uuid.UUID | None = None) -> list:
    ...
    if filter_user_id:
        return [
            AnalyticsEvent.video_id.in_(
                select(Video.id).where(Video.owner_id == filter_user_id)
            )
        ]
    return []
...
async def get_summary(
    ...
    user_id: uuid.UUID | None = Query(default=None, description="Admin only: filter by user"),  # noqa: B008
) -> AnalyticsSummary:
    is_admin = current_user.role == UserRole.ADMIN
    filter_user_id = user_id if is_admin else None
    ckey = _cache_key(str(current_user.id), is_admin, str(filter_user_id) if filter_user_id else None)
